...
#include "ec_maca_reduce_ops.h"
#include <maca_cooperative_groups.h>
using namespace cooperative_groups;

#define align_pow2(_n, _p) ((_n) & ((_p) - 1))

__global__ void executor_start(ucc_ec_maca_executor_state_t *state,
                               int *cidx)
{
    *cidx  = 0;
...
__global__ void executor_shutdown_ack(ucc_ec_maca_executor_state_t *state)
{
...
#define LAUNCH_REDUCE_A(NAME, _Type, _AlphaType, _task, _unroll, ...)          \
    do {                                                                       \
        if (_task->task_type == UCC_EE_EXECUTOR_TASK_REDUCE) {                 \
            ucc_reduce_maca_##NAME<_Type, _AlphaType, true, false, _unroll,    \
                                   ucc_eee_task_reduce_t>(_task->reduce,       \
                                                          _task->flags);       \
        } else {                                                               \
            ucc_reduce_maca_##NAME<_Type, _AlphaType, true, true, _unroll,     \
...
__device__ ucc_status_t executor_reduce(ucc_ee_executor_task_args_t *task)
{
...
#if SIZEOF_MCFLOATCOMPLEX == 8
        DT_REDUCE_FLOAT_COMPLEX(mcFloatComplex, float, task, op,
...
#if SIZEOF_MCDOUBLECOMPLEX == 16
        DT_REDUCE_FLOAT_COMPLEX(mcDoubleComplex, double, task, op,
    ...
    case UCC_DT_BFLOAT16:
        ucc_assert_system(2 == sizeof(__maca_bfloat16));
        DT_REDUCE_FLOAT(__maca_bfloat16, task, op,
                        REDUCE_LOOP_UNROLL_TRIGGERED_TWO);
        break;
    default:
        return UCC_ERR_NOT_SUPPORTED;
...
__global__ void executor_kernel(volatile ucc_ec_maca_executor_t *eee,
                                int q_size)
{
    ...
    if (is_master) {
        if (useCoopLaunch) {
            *eee->dev_cidx  = 0;
            *eee->dev_state = UCC_EC_MACA_EXECUTOR_STARTED;
        }
        ...
        if (worker_done) {
            if (useCoopLaunch) {
                ...
                if (is_master && (worker_id == 0)) {
                    *eee->dev_state = UCC_EC_MACA_EXECUTOR_SHUTDOWN_ACK;
                }
            }
            return;
        }
...
{
    mcStream_t stream = (mcStream_t)eee->super.ee_context;
    int          nb     = EC_MACA_CONFIG->exec_num_workers;
    int          nt     = EC_MACA_CONFIG->exec_num_threads;
    int          q_size = EC_MACA_CONFIG->exec_max_tasks;
    int          useCoopLaunch = EC_MACA_CONFIG->use_cooperative_launch;

    if (useCoopLaunch) {
        void *kernelArgs[] = {&eee, &q_size};
        dim3  dimBlock(nt, 1, 1);
        dim3  dimGrid(nb, 1, 1);
        mcLaunchCooperativeKernel((void *)executor_kernel<true>, dimGrid, dimBlock,
                                    kernelArgs, 0, stream);
    } else {
...
ucc_status_t ucc_ec_mc_copy_multi_kernel(const ucc_ee_executor_task_args_t *args,
                                           mcStream_t stream)
{
    int nt = 1024;
