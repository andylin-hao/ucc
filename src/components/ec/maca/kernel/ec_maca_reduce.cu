...
template <>
__global__ void
UCC_REDUCE_MACA_MULTI_DST_SUM<float, false>(ucc_eee_task_reduce_multi_dst_t arg)
{
...
#define LAUNCH_REDUCE_A(NAME, type, _AlphaType, _task, s, b, t)                \
    do {                                                                       \
        if (_task->task_type == UCC_EE_EXECUTOR_TASK_REDUCE) {                 \
            UCC_REDUCE_MACA_DEFAULT_##NAME<type, _AlphaType, false,            \
            ...
            UCC_REDUCE_MACA_STRIDED_##NAME<type, _AlphaType, false,            \
                                           REDUCE_LOOP_UNROLL_INTERRUPTIBLE>   \
                <<<b, t, 0, s>>>(_task->reduce_strided, _task->flags);         \
        } else {                                                               \
            UCC_REDUCE_MACA_MULTI_DST_##NAME<type, false>                      \
...
ucc_status_t ucc_ec_maca_reduce(ucc_ee_executor_task_args_t *task,
                                mcStream_t                 stream)
{
    int                th = EC_MACA_CONFIG->reduce_num_threads;
    unsigned long      bk = EC_MACA_CONFIG->reduce_num_blocks;
    ...
    case UCC_DT_FLOAT32_COMPLEX:
        ...
        DT_REDUCE_FLOAT_COMPLEX(mcFloatComplex, float, task, op, stream, bk, th);
        break;
#else
        return UCC_ERR_NOT_SUPPORTED;
#endif
    case UCC_DT_FLOAT64_COMPLEX:
        ...
        DT_REDUCE_FLOAT_COMPLEX(mcDoubleComplex, double, task, op, stream, bk, th);
        break;
#else
        return UCC_ERR_NOT_SUPPORTED;
#endif
    case UCC_DT_BFLOAT16:
        ...
        DT_REDUCE_FLOAT(__maca_bfloat16, task, op, stream, bk, th);
        break;
    default:
        ec_error(&ucc_ec_maca.super, "unsupported reduction type (%s)",
                 ucc_datatype_str(dt));
        return UCC_ERR_NOT_SUPPORTED;
    }
