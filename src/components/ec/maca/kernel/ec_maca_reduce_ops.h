#ifndef UCC_EC_MACA_REDUCE_OPS_H_
#define UCC_EC_MACA_REDUCE_OPS_H_
...
__device__ inline mcDoubleComplex operator+(const mcDoubleComplex &first,
                                            const mcDoubleComplex &second)
{
    return mcCadd(first, second);
}

__device__ inline mcDoubleComplex operator*(const mcDoubleComplex &first,
                                            const mcDoubleComplex &second)
{
    return mcCmul(first, second);
}

__device__ inline mcDoubleComplex operator*(const mcDoubleComplex &first,
                                            const double &         second)
{
    return make_mcDoubleComplex(mcCreal(first) * second,
                                mcCimag(first) * second);
}

__device__ inline mcFloatComplex operator+(const mcFloatComplex &first,
                                           const mcFloatComplex &second)
{
    return mcCaddf(first, second);
}

__device__ inline mcFloatComplex operator*(const mcFloatComplex &first,
                                           const mcFloatComplex &second)
{
    return mcCmulf(first, second);
}

__device__ inline mcFloatComplex operator*(const mcFloatComplex &first,
                                           const float &         second)
{
    return make_mcFloatComplex(mcCrealf(first) * second,
                               mcCimagf(first) * second);
}

template <typename T, typename VecType> __device__ int ptrAlignVec(T *ptr)
{
    return (uintptr_t)ptr % sizeof(VecType);
}
...
#define MACA_REDUCE_WITH_OP_CHUNK(unroll, unroll_group_size, _OP, VecType) \
    do {                                                                       \
...
#define MACA_REDUCE_WITH_OP(NAME, _OP)                                          \
    template <typename Type, typename AlphaType, bool triggered, bool strided,  \
              int UNROLL, typename TaskType>                                    \
    __device__ void ucc_reduce_maca_##NAME(TaskType task, uint16_t flags)       \
    {                                                                           \
        ...
        /* Successive calls to MACA_REDUCE_WITH_OP_CHUNK to reduce the buffer.*/\
        ...
        if (triggered && alignedVec == 0) {                                     \
            ...
            MACA_REDUCE_WITH_OP_CHUNK(UNROLL, WARP_SIZE, _OP, vectype);         \
            /* call with vectorization but without unrolling */                 \
            MACA_REDUCE_WITH_OP_CHUNK(1, 1, _OP, vectype);                      \
        }                                                                       \
        /* call with unrolling but without vectorization */                     \
        MACA_REDUCE_WITH_OP_CHUNK(UNROLL, WARP_SIZE, _OP, Type);                \
        /* last call without unrolling nor vectorization */                     \
        MACA_REDUCE_WITH_OP_CHUNK(1, 1, _OP, Type);                             \
    }                                                                           \
    template <typename Type, typename AlphaType, bool triggered, int UNROLL>    \
    __global__ void UCC_REDUCE_MACA_DEFAULT_##NAME(ucc_eee_task_reduce_t task,  \
                                                   uint16_t              flags) \
    {                                                                           \
        ucc_reduce_maca_##NAME<Type, AlphaType, triggered, false, UNROLL,       \
                               ucc_eee_task_reduce_t>(task, flags);             \
    }                                                                           \
    template <typename Type, typename AlphaType, bool triggered, int UNROLL>    \
    __global__ void UCC_REDUCE_MACA_STRIDED_##NAME(                             \
        ucc_eee_task_reduce_strided_t task, uint16_t flags)                     \
    {                                                                           \
        ucc_reduce_maca_##NAME<Type, AlphaType, triggered, true, UNROLL,        \
                               ucc_eee_task_reduce_strided_t>(task, flags);     \
    }

#define MACA_REDUCE_WITH_OP_MULTI_DST(NAME, _OP)                               \
    template <typename _Type, bool triggered>                                  \
    __global__ void UCC_REDUCE_MACA_MULTI_DST_##NAME(                          \
...
MACA_REDUCE_WITH_OP(SUM, DO_OP_SUM);
MACA_REDUCE_WITH_OP(PROD, DO_OP_PROD);
MACA_REDUCE_WITH_OP(MIN, DO_OP_MIN);
MACA_REDUCE_WITH_OP(MAX, DO_OP_MAX);
MACA_REDUCE_WITH_OP(LAND, DO_OP_LAND);
MACA_REDUCE_WITH_OP(LOR, DO_OP_LOR);
MACA_REDUCE_WITH_OP(LXOR, DO_OP_LXOR);
MACA_REDUCE_WITH_OP(BAND, DO_OP_BAND);
MACA_REDUCE_WITH_OP(BOR, DO_OP_BOR);
MACA_REDUCE_WITH_OP(BXOR, DO_OP_BXOR);

MACA_REDUCE_WITH_OP_MULTI_DST(SUM,  DO_OP_SUM);
MACA_REDUCE_WITH_OP_MULTI_DST(PROD, DO_OP_PROD);
MACA_REDUCE_WITH_OP_MULTI_DST(MIN,  DO_OP_MIN);
MACA_REDUCE_WITH_OP_MULTI_DST(MAX,  DO_OP_MAX);
MACA_REDUCE_WITH_OP_MULTI_DST(LAND, DO_OP_LAND);
MACA_REDUCE_WITH_OP_MULTI_DST(LOR,  DO_OP_LOR);
MACA_REDUCE_WITH_OP_MULTI_DST(LXOR, DO_OP_LXOR);
MACA_REDUCE_WITH_OP_MULTI_DST(BAND, DO_OP_BAND);
MACA_REDUCE_WITH_OP_MULTI_DST(BOR,  DO_OP_BOR);
MACA_REDUCE_WITH_OP_MULTI_DST(BXOR, DO_OP_BXOR);
...
#endif // UCC_EC_MACA_REDUCE_OPS_H_