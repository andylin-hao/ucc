...
ucc_status_t ucc_maca_executor_persistent_task_post(
    ...
    ucc_ee_executor_task_t **task)
{
    ...
    ucc_ec_maca_executor_persistent_task_t *ee_task;
    int                                     i;
    ...
    ee_task = ucc_mpool_get(&ucc_ec_maca.executor_persistent_tasks);
    ...
    case UCC_EE_EXECUTOR_TASK_COPY_MULTI:
        ...
        for (i = 0; i < ee_task->num_subtasks; i++) {
            ucc_ee_executor_task_args_t *subtask_args =
                &(eee->tasks[(eee->pidx + i) % max_tasks]);
    ...
    ec_debug(&ucc_ec_maca.super, "executor task post, eee: %p", eee);

    *task = &ee_task->super;
    return UCC_OK;
}

ucc_status_t ucc_maca_executor_persistent_task_test(const ucc_ee_executor_task_t *task)
{
    ucc_ec_maca_executor_persistent_task_t *ee_task;
    ucc_status_t status;
    int i;

    ee_task = ucc_derived_of(task, ucc_ec_maca_executor_persistent_task_t);

    if (ee_task->super.status != UCC_INPROGRESS) {
        goto exit;
    }

    status = MACA_FUNC(mcGetLastError());
...
ucc_status_t ucc_maca_executor_persistent_task_finalize(ucc_ee_executor_task_t *task)
{
...
ucc_status_t ucc_maca_executor_persistent_start(ucc_ee_executor_t *executor, void *ee_context) {
    ucc_ec_maca_executor_t *eee = ucc_derived_of(executor, ucc_ec_maca_executor_t);
    ucc_status_t status;

    ucc_assert(eee->state == UCC_EC_MACA_EXECUTOR_INITIALIZED);
    ec_debug(&ucc_ec_maca.super, "executor start, eee: %p", eee);
    eee->super.ee_context = ee_context;
    eee->state = UCC_EC_MACA_EXECUTOR_POSTED;
    eee->pidx = 0;
    eee->mode = UCC_EC_MACA_EXECUTOR_MODE_PERSISTENT;

    status = ucc_ec_maca_persistent_kernel_start(eee);
    if (status != UCC_OK) {
        ec_error(&ucc_ec_maca.super, "failed to launch executor kernel");
        return status;
    }

    eee->ops.task_post = ucc_maca_executor_persistent_task_post;
    eee->ops.task_test = ucc_maca_executor_persistent_task_test;
    eee->ops.task_finalize = ucc_maca_executor_persistent_task_finalize;
    return UCC_OK;
}

ucc_status_t ucc_maca_executor_persistent_stop(ucc_ee_executor_t *executor)
{
    ucc_ec_maca_executor_t *eee = ucc_derived_of(executor, ucc_ec_maca_executor_t);
