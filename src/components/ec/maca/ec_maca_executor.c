...
ucc_status_t ucc_maca_executor_persistent_start(ucc_ee_executor_t *executor,
                                                void *             ee_context);
...
ucc_status_t
ucc_maca_executor_persistent_wait_stop(ucc_ee_executor_t *executor);

ucc_status_t ucc_maca_executor_init(const ucc_ee_executor_params_t *params,
                                    ucc_ee_executor_t **            executor)
{
    ucc_ec_maca_executor_t *eee = ucc_mpool_get(&ucc_ec_maca.executors);

    if (ucc_unlikely(!eee)) {
        ec_error(&ucc_ec_maca.super, "failed to allocate executor");
        return UCC_ERR_NO_MEMORY;
    }

    /* Only used for synchronization before checking CC progress */
    ...
    ec_debug(&ucc_ec_maca.super, "executor init, eee: %p", eee);
...
ucc_status_t ucc_maca_executor_status(const ucc_ee_executor_t *executor)
{
    ...
    switch (eee->state) {
    case UCC_EC_MACA_EXECUTOR_INITIALIZED:
        return UCC_OPERATION_INITIALIZED;
    case UCC_EC_MACA_EXECUTOR_POSTED:
        return UCC_INPROGRESS;
    case UCC_EC_MACA_EXECUTOR_STARTED:
        return UCC_OK;
    default:
...
ucc_status_t ucc_maca_executor_finalize(ucc_ee_executor_t *executor)
{
    ...
    ec_debug(&ucc_ec_maca.super, "executor free, eee: %p", eee);
...
ucc_status_t
ucc_maca_executor_task_post(ucc_ee_executor_t *                executor,
...
{
    ucc_ec_maca_executor_t *eee =
        ucc_derived_of(task->eee, ucc_ec_maca_executor_t);
                                     ...
                                     void *             ee_context)
{
    ...
    if (!ee_context) {
        return ucc_maca_executor_interruptible_start(executor);
    } else {
        if (eee->requested_ops == 0) {
            /* no operations requested, just mark stream busy */
            return ucc_maca_executor_persistent_wait_start(executor,
                                                           ee_context);
        } else {
            return ucc_maca_executor_persistent_start(executor, ee_context);
        }
    }
}
...
ucc_status_t ucc_maca_executor_stop(ucc_ee_executor_t *executor)
{
    ...
    if (eee->mode == UCC_EC_MACA_EXECUTOR_MODE_INTERRUPTIBLE) {
        return ucc_maca_executor_interruptible_stop(executor);
    } else {
        if (eee->requested_ops == 0) {
            return ucc_maca_executor_persistent_wait_stop(executor);
        } else {
            return ucc_maca_executor_persistent_stop(executor);
        }
    }
}